...
3.定义reducer languageReducer.ts
    import {
        CHANGE_LANGUAGE,
        ADD_LANGUAGE,
        LanguageActionTypes,
...
6.将header.tsx与store隔离开，不耦合，避免每次使用组件时，都要指定hook的类型   hooks.ts
...
```react
1.安装https://react.i18next.com/guides/quick-start
...
2.创建一个配置文件config.ts
    ...
    const resources = {
        en: {
            translation: translation_en, //对应的指定json文件
        },
        zh: {
...
4.提供全局的context
import "./i18n/config" //只需要导入i18n配置，默认提供全局使用的context
5.在类组件中使用全局语言切换数据
	在类组件中使用共享数据需要使用高阶函数组件来传递到props中
    // 引入高阶函数，来使用全局组件的数据
    import  {withTranslation,WithTranslation} from 'react-i18next'//withTranslation高阶组件 	//	WithTranslation定义的类型
     ...
     export const HomePage = withTranslation()(HomePageComponent) //需要用高阶组件包裹
6.函数组件中使用
//使用对应的hooks就可以提取到全局共享的数据
import React from "react";
...
7.实现语言切换
import i18n from "i18next";
i18n.changeLanguage('zh'); // 这样处理是不标准的，有副作用 传递resource里面定义的key值
```
